...
import { fetchPriductsThree } from "../api/productsApi";
...
const ProductsListThree = () => {
  ...
  if (isLoading) {
    return (
      <p style={{marginTop: "200px", marginLeft: "700px", display: "flex", alignItems: "center", gap: "10px" }}>
        <Loader color="blue" /> Muddatli to'lov tovarlari yuklanmoqda...
      </p>
    );
  }
  if (error) {
    return (
      <p style={{ textAlign: "center" }}>
        Muddatli to'lov mahsulotlarini olib kelishda xatolik yuz berdi:{" "}
  ...
  const toggleWishList = (item) => {
    const exists = wishList.some((mahs) => mahs.id === item.id);
    if (exists) {
      setWishList(wishList.filter((mahs) => mahs.id !== item.id));
    } else {
  ...
  return (
    <div>
      <List>
        <Title c={"#1f2026b9"} mt={80} mb={20}>
          Muddatli to'lov <FaChevronRight size={22} />
        </Title>
...
export default ProductsListThree;